...
export class ReactCollectionWrapper<
  Schema extends CollectionSchema,
  DBFullSchema extends DatabaseFullSchema = DatabaseFullSchema
> implements IReactCollectionWrapper<false, Schema, DBFullSchema>
{
  constructor(
    private collection: Collection<false, Schema, DBFullSchema>,
    private queriesManager: QueriesManager<Signal>
  ) {}
  ...
  >(query: Q) {
    ...
    if (this.queriesManager.has(queryHash)) {
      return this.queriesManager.subscribe(queryHash) as Signal<
    ...
    return this.queriesManager.set(
      ...
      query,
      signal(queryResult)
    ) as Signal<QueryResult<Schema, DBFullSchema, Q>[0]>;
  }
  ...
  findMany<Q extends Query<Schema, DBFullSchema> = Query<Schema, DBFullSchema>>(
    ...
    return this.queriesManager.set(
      ...
      query,
      signal(queryResult)
    ) as Signal<QueryResult<Schema, DBFullSchema, Q>>;
  }
