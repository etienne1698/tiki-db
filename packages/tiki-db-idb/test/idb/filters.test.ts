...
test("$in should work", async () => {
  ...
  expect(
    (await db.collections.users.findMany({ filters: { id: { $in: ["1", "2"] } } }))
...
test("$ne should work", async () => {
  ...
  expect(
    (await db.collections.users.findMany({ filters: { id: { $ne: "2" } } }))
...
test("$notIn should work", async () => {
  ...
  expect(
    (await db.collections.users.findMany({ filters: { id: { $notIn: ["1", "3"] } } }))
  ).toContainEqual({ id: "2", firstname: "Geoffroy" });
